...
public interface GrupoRepository extends JpaRepository<Grupo, Integer> {
	  ...
	  @Query(value = "SELECT * FROM Grupo g JOIN Aluno a ON g.codigo = a.codigoGrupo WHERE a.nome LIKE %:nomeIntegrante%", nativeQuery = true)
	  List<Grupo> findByNomeIntegrante(@Param("nomeIntegrante") String nomeIntegrante);

	   
	   @Query(value ="SELECT * FROM Grupo g JOIN Professor p ON g.codigoProfessor = p.codigo WHERE p.nome LIKE %:nomeProfessor%", nativeQuery = true)
